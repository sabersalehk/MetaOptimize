...
parser.add_argument('data', metavar='DIR', nargs='?', default='ILSVRC2012/',
                    help='path to dataset (default: imagenet)')
parser.add_argument("--save-directory", type=str, default='data',
...
parser.add_argument('--optimizer', metavar='OPT', nargs='?', default='SGD',
...
parser.add_argument('--dummy', default=False, action='store_true', help="use fake data to benchmark")
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    run_name = args.run_name if args.run_name is not None else f"{args.optimizer}__{args.lr}__{args.seed}__{int(time.time())}"
    ...
    if args.dummy:
        ...
        train_dataset = datasets.FakeData(100000, (3, 224, 224), 1000, transforms.ToTensor()) #1281167
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        acc1, acc5, losses = train(train_loader, model, criterion, optimizer, epoch, device, args)


        writer.add_scalar("Performance/train_acc1", acc1.cpu().numpy(), epoch)
        writer.add_scalar("Performance/train_acc5", acc5.cpu().numpy(), epoch)
        writer.add_scalar("Performance/train_losses", losses.val, epoch)

        # evaluate on validation set
        acc1 = validate(val_loader, model, criterion, args)
        writer.add_scalar("Performance/val_acc1", acc1.cpu().numpy(), epoch)
