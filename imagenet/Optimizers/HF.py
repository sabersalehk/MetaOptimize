        ...
        if self.counter%100==0 and True:
            if self.stepsize_type == 'scalar':
                ...
                for i in range(self.num_blocks):
                    self.writer.add_scalar("Optimizer_blockwise/beta_block"+str(i), np.exp(self.beta[0][i].item()), self.counter)
...
###################################################
    ...
    def init_meta(self, stepsize_groups, net_param_names_and_size, alpha0):
        ...
        if self.stepsize_type == 'scalar':
            ...
            self.beta = [torch.log(alpha0) * torch.ones(len(stepsize_groups))]
