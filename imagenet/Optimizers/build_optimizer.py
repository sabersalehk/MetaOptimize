...
def build_optimizer(net, args, writer):
    ...
    if args.optimizer == 'SGD':
        return SGD_optimizer(net, lr=args.base_lr, writer=writer)
    if args.optimizer == 'SGDm':
        return SGDm_optimizer(net, lr=args.base_lr, momentum=args.base_momentum, writer=writer)
    if args.optimizer == 'Adam':
        return Adam_optimizer(net, lr=args.base_lr, writer=writer)
    if args.optimizer == 'AdamW':
        return AdamW_optimizer(net, lr=args.alpha0, writer=writer)
    if args.optimizer == "Lion":
        return Lion_optimizer(net, lr=args.alpha0, weight_decay=args.weight_decay_base, writer=writer)
...
class SGD_optimizer():
    def __init__(self, net, lr, writer=None):
        self.lr = lr
        self.optimizer = torch.optim.SGD(net.parameters(), lr=lr)
        self.scheduler = StepLR(self.optimizer, step_size=500*30, gamma=0.1)


    def step(self, net, loss, data=None):
        net.zero_grad()
        loss.backward()
        self.optimizer.step()
        self.scheduler.step()
...
class SGDm_optimizer():
    def __init__(self, net, lr, momentum, writer=None):
        self.lr = lr
        self.momentum = momentum
        self.optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=momentum)
        self.scheduler = StepLR(self.optimizer, step_size=500*30, gamma=0.1)
...
class Lion_optimizer():
    ...
    def __init__(self, net, lr, weight_decay, writer=None):
        self.total_steps = 422000 # Define total training steps
        self.warmup_steps = 10000  # Define warmup steps
        self.num_layers = len([0 for _ in  net.parameters()])
        self.momentum = [0.0 for _ in  range(self.num_layers)]
        self.counter = 0
        self.base_lr = lr
        self.min_lr = 0
    ...
    def get_lr(self):
        if self.counter < self.warmup_steps:
            return self.base_lr * (self.counter / self.warmup_steps) 
        else:
            self.finished_warmup = True
            return self.min_lr + (self.base_lr - self.min_lr) * (1 + math.cos(math.pi * (self.counter - self.warmup_steps) / (self.total_steps - self.warmup_steps))) / 2 
    ...
    def step(self, net, loss):
        ...
        for w, grad, moment, i in zip(net.parameters(), g, self.momentum, range(self.num_layers)):
            delta_w = lr * (torch.sign(0.9 * moment + 0.1*grad) + self.weight_decay * w.data)
            w.data = w.data -  delta_w
            self.momentum[i] = 0.99 *self.momentum[i] + (0.01)*grad     
