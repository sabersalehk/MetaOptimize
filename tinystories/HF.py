...
class HF():
    ...
    def __init__(self, net, stepsize_groups, alpha0, args_base, args_meta, gamma, writer=None):
        '''
        stepsize_groups: in ['scalar', 'layerwise', 'nodewise', 'weightwise', 'resnet18_blocks', 'resnet50_blocks', or [[name_layer1, name_layer2,...], [name_layer_i,...],...], or [int_1, int_2,...] where int_i=size_of_group_i ]
    ...
    def step(self, net, loss):
        ...
        if self.counter%100==0 and True:
            if self.stepsize_type == 'scalar':
                ...
                for (name,_),i in zip(net.named_parameters(), range(self.num_layers)):
                    self.writer.add_scalar("Optimizer_layerwise/alpha_layer_"+name, np.exp(self.beta[0][i].item()), self.counter)
                    self.writer.add_scalar("Optimizer_layerwise/beta_layer_"+name, self.beta[0][i].item(), self.counter)
    ...
    def polish_the_stepsize_groups(self, stepsize_groups,net_param_names_and_size):
        ...
        elif stepsize_groups == 'transformer_blocks_15m':
            stepsize_groups = [1,9,9,9,9,9,10]
